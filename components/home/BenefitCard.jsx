...
export default function BenefitCard({
  ...
  return (
    ...
    >
      <div
        className={`flex h-44 items-center justify-center bg-gradient-to-br ${accent} sm:h-52`}
        aria-hidden
      >
        <span className="text-5xl sm:text-6xl">{icon}</span>
      </div>
      <div className="flex flex-1 flex-col p-6 sm:p-8">
        <h3 className="text-xl font-bold text-[#1a3a4a] sm:text-2xl">{title}</h3>
        <p className="mt-3 flex-1 leading-relaxed text-slate-600">{description}</p>
        <Link
          href={href}
          className="mt-6 inline-flex w-fit items-center text-sm font-semibold text-[#2d6a7e] transition hover:text-[#245663]"
        >
          بیشتر بدانید
